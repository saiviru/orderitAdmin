  ...
  useEffect(() => {
    if (orderData && orderData.length > 0) {
      ...
      const ordersByHour = orderData.reduce((result, order) => {
        ...
        result[hour12].orders.push(order);
        result[hour12].totalAmount += order.totalAmount;
        return result;
      }, {});

      // Arrange orders based on labels
      const arrangedOrders = Rawdata.labels.map(label => {
        const hour = parseInt(label.replace(/[a-z]/gi, ''), 10);

        const { orders, totalAmount } = ordersByHour[hour] || { orders: [], totalAmount: 0 };
